...
```{r}
random_sample1 <- random_sample1[, ':=' (uid=NULL,duration=as.numeric(duration),orig_bytes=as.numeric(orig_bytes),resp_bytes=as.numeric(resp_bytes),local_orig=NULL,tunnel_parents=NULL)]
```
...
Here we set all the Nas to 0 and change the malcious column to mal which uses 1 to represent malicous == TRUE and 0 otherwise. These are for xgboost to work.
```{r}
