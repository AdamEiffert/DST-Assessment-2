...
```{r}
set.seed(63)
sample <- createDataPartition(final_df$mal, p = 0.7, list = FALSE)
...
```{r}
#we use a gradient boost tree for xgboost, our objective is binary:logistic as we are classifying nonrmal (0) vs attack (1), 
...
#our prediction gives us values 0-1 and so we want to return 1 if binary:logistic > 0.5 and 0 otherwise
